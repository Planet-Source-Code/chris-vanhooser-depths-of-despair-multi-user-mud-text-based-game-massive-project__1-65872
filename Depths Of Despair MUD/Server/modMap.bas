...
If LCaseFast(X(Index)) = "map" Then
        ...
        For j = 0 To 10
            With udtMaparea(i, j)
                If .sIsRoom Then
                    ...
                    If .dN <> 0 Then Arr(h) = Arr(h) & "º" Else Arr(h) = Arr(h) & " "
                    If .dNE <> 0 Then Arr(h) = Arr(h) & "/" Else Arr(h) = Arr(h) & " "
                    If .dW <> 0 Then Arr(h + 1) = Arr(h + 1) & "Í" Else Arr(h + 1) = Arr(h + 1) & " "
                    If .lRealID = dbPlayers(dbIndex).lLocation Then
                        Arr(h + 1) = Arr(h + 1) & BRIGHTRED & "²" & BRIGHTWHITE
                    Else
                        Arr(h + 1) = Arr(h + 1) & "²"
                    End If
                    If .ddE <> 0 Then Arr(h + 1) = Arr(h + 1) & "Í" Else Arr(h + 1) = Arr(h + 1) & " "
                    If .dSW <> 0 Then Arr(h + 2) = Arr(h + 2) & "/" Else Arr(h + 2) = Arr(h + 2) & " "
                    If .dS <> 0 Then Arr(h + 2) = Arr(h + 2) & "º" Else Arr(h + 2) = Arr(h + 2) & " "
    ...
    For i = LBound(Arr) To UBound(Arr)
        Arr(i) = YELLOW & "º " & BRIGHTWHITE & Arr(i) & YELLOW & " º"
    Next
'ÉÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ»
'º                                 º
'º\|/\|/\|/\|/\|/\|/\|/\|/\|/\|/\|/º
...
'ÈÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¼
    s = Join(Arr, vbCrLf)
    s = YELLOW & "ÉÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ»" & vbCrLf & _
        "º                                   º" & vbCrLf & _
        s & vbCrLf & _
        "º                                   º" & vbCrLf & _
